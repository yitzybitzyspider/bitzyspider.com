...
**Bosses** (every 5th level via `pickBoss()`, alternating): **Hornet Queen** (L5, 15, …: 5 HP, orbits screen center, spawns up to 3 wasps marked `queenSpawn`) and **Mantis** (L10, 20, …: 4 HP, stalks at ~180px, lunges at 1.5x speed to sting, seeks strands to tear). True bosses have `boss: true` (hub-immune, tears strands, qv 5). HP pips render in the boss's theme color (`BOSS_COLORS`), filled/empty.

...
**World themes (v28):** a **WORLD picker row** on the menu (`th_*` buttons, same `.sizeBtn` styling as the arena row) selects one of five backdrops, persisted as `bughunt_theme` and applied instantly via `setTheme()` → `renderBg()`. The `THEMES` table parametrizes the sky engine — `hue(lv)` (per-level rotation inside the theme's color family), `sat`, `neb` (nebula count, 0 disables), `nebSpread` (hue variance), `stars` (density multiplier), plus one signature layer each: **COSMIC** (the original full-spectrum space, hue +33°/level), **NEON** (`grid: true` — synthwave horizon + converging perspective grid in the theme hue and its complement, purple family), **GARDEN** (`fireflies: true` — baked warm glow-dots, deep greens), **EMBER** (`emberGlow: true` — warm radial glow rising from the bottom edge, fire hues), **VOID** (minimal: near-mono, no nebulas, sparse stars). Level-to-level color rotation still happens *within* each theme's family.

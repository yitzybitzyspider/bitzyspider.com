...
**Living Silk (v30 — web building as the core fun loop):** four base mechanics + two upgrades make weaving self-rewarding. **Wall anchors:** a strand endpoint within 24px of the arena edge grips the world itself — counts as 2 attachments in `recalcSupport` (never frays), flagged `a1`/`a2` and drawn as a stud (silver dot + red pip). Corner-to-corner spans are real architecture. **Silk maturity** (`silkTier(st,t)` from `st.born`, `SILK_MAT = [1, 1.4, 2]`): surviving (non-fraying) strands **season at 12s** (+40% stuck time, brighter) and turn **gilded at 30s** (gold shimmer, 2× stuck time, and an escaping bug no longer snaps the strand — it survives and drops back to seasoned with a "silk held!" float). **Silk highways:** in `moveSpider`, within 12px of an own strand the movement component parallel to it is boosted ×1.2 (glide streak particles) — the web is a transit network. **Hub web bonus:** wraps within `HUB_R*1.2` of a hub pay ×1.5 points with a golden "WEB BONUS!" float. **New shop upgrades:** SILK HIGHWAYS (`hw`, max 2, +0.15×/tier on the glide) and DEW HARVEST (`dew`, max 2, hubs trickle +1.2 webSilk/s per tier, capped at 3 hubs). Glossary gained a "Living Silk" card; Strand Support card mentions wall anchors.

**Web defense & repair (v31):** the auto-weaver (Q) is **ADMIN-only** now — hand-weaving is the game (`toggleAutoWeb` gates on `ADMIN`; the AUTO pill hides; CTRL_ROWS dropped the row; the walk-the-edges machinery survives for repair + admin). **Repair reworked:** `fallenLog` remembers the last 12 strands that fell (`recordFall` from `breakStrand`, burn removals, and decay) and **R re-weaves exactly what fell**, newest first, up to 8 per pass; with nothing fallen it refuses ("the web stands"). **Attackers vs maturity:** fireflies **scorch** gilded silk (survives, `born` resets to fresh, "gilded silk held!") instead of burning it; the **webworm hunts your most mature strand first** and chews gilded for 2.4s instead of 1.2s; a dragonfly/boss tear on gilded silk is **survived** (drops to seasoned) and **tangles** the brute (`dragUntil` 700ms at 0.35×). **Tear fatigue (v31.1):** a tearer can only break one strand per 450ms (`b.tearAt` gate) and each tear staggers it 300ms; strands it hits during the cooldown **snag** it (320ms at 0.4×) instead of breaking — dense webs bog a dragonfly down. The first tear in 4s raises a **\"WEB UNDER ATTACK!\"** cue (`webAlarmAt`). While a tearer/boss is tangled (`dragUntil` active), **web shots deal double damage** (`webBug` extra `hp--`, \"TANGLED! x2\" float) — the counterplay is to feed it silk, then punish it. Multi-bug erosion raised to 40%/extra bug. **SILK SURGE:** the silk-droplet gem now refills **35% of both meters** and makes weaving **free for 4s** (`surgeUntil` gates `strandCost`; HUD shows `· SURGE Ns`). **ROYAL LOOM** (advanced sink, max 3): dyes fresh silk moonlight/amethyst/blood-red (cosmetic; seasoned stays bright, gilded stays gold). Visual polish: hub rings softened (2 rings at 0.09 alpha + a pearl at the heart), crossings draw one clean stitched diamond instead of the triple weave. Tutorial: AUTO-WEB step replaced by **HUB** (join 3 strands) and **ANCHOR** (pin an end at the wall); REPAIR step breaks two real strands and teaches R.
...
**Pacing & juice (v24):** spawn pacing got three fixes — `maxAlive` scales with the arena (+2 on areas > 620k px² so FULL screens don't feel sparse), a **catch-up spawner** (interval × 0.55 when the field is under 40% of maxAlive, so there's never dead air), and **swarm bursts** (from level 3, every 16–26s a wave of 3–6 flies/moths floods in at once — allowed to exceed `maxAlive` — with a big "SWARM!" banner; `swarmAt` timer). **Gem pickups:** wrapped non-boss bugs have a 12% chance to drop a small walk-over pickup (`gems` array, `spawnGem`/`updateGems` — `spawnGem(x,y,force)` takes an optional forced kind, 8s lifetime with an expiring blink): silk droplet (55%, +25 to both meters), point gem (25%, +15×level), power star (15%, grants a random temporary shot mode — see v25), heart (5%, only when hurt). **Feedback:** `floatTxt` takes a `big` flag (22px + drop shadow) used for level starts, boss names, SWARM!, RAGE READY!, and combos ≥3; a "N TO GO!" cue fires once per level when ≤3 bugs remain (`quotaCued`); plain snags now ripple too. **Graphics:** every bug and the spider cast a soft ground shadow, true bosses radiate a pulsing aura in their theme color (rogues get a faint one), and strands render in two passes (wide faint halo under a thin bright core) so silk reads as gossamer.

**Weapons & boss overhaul (v25/v26):** shooting got a real arsenal and bosses got teeth. **Weapons** (`SHOT_MODES`, `shotMode`/`shotModeUntil`, `shotModeActive()`, `grantShotMode(k)`, 12s via `SHOT_MODE_DUR`) — think gun pickups in a shooter, each changes how firing *feels*: **SHOTGUN** (`shotgun`, gold — 5-pellet fan at ±0.45 rad, pellets fly at 900 with 0.7× range: brutal up close), **LASER** (`laser`, cyan — a **continuous held beam** (v28): while the pointer is down, `update()` drains 30 silk/s and calls `laserFire()` every frame toward the cursor, so you sweep it in circles and everything it crosses gets webbed. Multi-HP bugs tick once per 350ms via `b.lzAt` so bosses melt steadily, not 60×/s. Beam segments push into `beams` each frame (life 0.55, fast fade → a sweeping trail); `SFX.laser` throttled via `laserSfxAt`. A quick tap still fires a single pulse through `shootWeb`, gated by `laserHeldAt` so a hold-release doesn't double-charge), **GATLING** (`gatling`, green — shots cost 6 silk instead of 30 AND **holding the pointer down auto-sprays** toward it every 110ms via the `gatlingAt` gate in `update()`), **BAZOOKA** (`bazooka`, red — **artillery** (v28): the rocket flies at 480 px/s **to the exact click point** (`s.tx/s.ty`, no homing, no range cap) and detonates there via `boomAt(x,y)`: `BOOM_R` 90px blast that webs everything in radius, twin ripples + `SFX.boom`. You aim the blast, not the projectile). Double Shot stacks its ±0.18 pair on top of any weapon except the bazooka (one big rocket is the identity). Granted by the **power star** gem (spinning orange 4-point star; 15% of gem rolls) and **always dropped by wrapped rogue bosses** — a rogue's star *replaces* its regular 12% gem roll (no double-dipping). Shot heads tint to the weapon color; the HUD silk line shows `· SHOTGUN 8s` countdowns; modes reset in `startGame`/`toMenu`/`startTutorial`. **Bosses:** HP now scales `+1 per 12 levels` (`b.maxHp`; pips render from `maxHp`), and at ≤`ceil(maxHp*0.4)` HP a true boss **ENRAGES** (`enrageCheck()` from `webBug`, banner + `SFX.rage`): 1.3× displacement speed, **stings on contact** (even bosses without base `sting`), minion caps go 3→4 (queen/mothking), and its aura flares red with a faster pulse. **Bug Bomb rework:** the bomb no longer insta-kills bosses (that cheese is gone) — it's now a **screen-wide shockwave** that fully wraps every non-boss bug anywhere on screen and deals exactly 1 hit to bosses, with a triple-ripple + big banner. **BUY A LIFE is capped at 3** purchases (was unlimited).
...
**Run report + share (v31.2):** `runMaxCombo/runBossKills/runMaxStrands/runMaxGilded/runWeapons` track the run (reset in `startGame`; combo/boss in `catchBug`, strand peak in `recalcSupport`, gilded peak counted in the strand draw loop, weapons in `grantShotMode`). `gameOver()` renders them into `#oStats` ("BIGGEST COMBO x4 · 2 BOSSES DOWN · WEB PEAK 12 STRANDS (2 gilded) · 3 WEAPONS WIELDED"), plays a descending `SFX.gameover` sting, and the **SHARE RUN** button (`#btnShare`) copies a brag line + game URL to the clipboard (navigator.clipboard with execCommand fallback; flips to "COPIED ✓"). The **shop** renders as a **two-column grid ≥700px** (`.shopCard` max-width 760px, `#shopRows` grid) since the upgrade list outgrew one column. `game.html` is now **indexable** with description/OG/twitter meta (card image `photos/bughunt-card.png`, a NEON boss-fight screenshot). **Tests moved into the repo** (`tests/harness.js` + `tests/hooks.js` + `tests/smoke.js`, run `node tests/smoke.js`, 51 checks) after scratchpad container recycles kept destroying the session-local suites.

**Menu/pause UI (v21 declutter):** the start menu is intentionally minimal — title, a one-line tagline, the BEST/BUGS-DEBUGGED stat (`#mHigh`/`#mTotal`), the arena-size picker, a big START HUNTING button, and a row of ghost buttons: **TUTORIAL, CONTROLS, FIELD GUIDE, HIGH SCORES, ♫ JUKEBOX**. The icon-based controls grid (`CTRL_ROWS` → `renderControls()`) is no longer inline on the menu; it lives in its own **CONTROLS overlay** (`#ovControls`, state `'controls'`, opened by `btnControlsMenu` via `openControls()`), which still renders into `#menuControls`. The pause overlay keeps its inline grid (`#pauseControls`). Touch and desktop get different rows. The old "KNOW YOUR BUGS" legend was dropped from the menu (it's redundant with the Field Guide's BESTIARY tab).
...
**Field Guide (v16):** a comprehensive glossary overlay (`#ovGloss`, state `'gloss'`) accessible from both the menu and pause screen via FIELD GUIDE ghost buttons. Three tabbed panes (`.glossTab` buttons switching `.glossPane` visibility): **BESTIARY** — every bug type with a live `drawBug` canvas preview (52×52), name (colored), threat tags (RAIDER/STINGER/BURNER/WEB-EATER/BOSS), the `FOE_INFO` description, and stat line (unlock level, escape time, weight class, special notes including boss drops). **UPGRADES** — all 9 core upgrades with icons (`UPG_ICONS`), pip count, and plain descriptions (`UPG_DESC`), plus the 4 advanced sinks. **MECHANICS** — 11 reference cards covering silk meters, strand support rules, heavy bugs, hubs, combos, brood/nest, difficulty scaling, boss drops, rage mode, shot power-ups, and enraged bosses. The `glossReturn` variable remembers whether to return to `'menu'` or `'pause'` on close. Rendering happens in `renderGlossary()` which populates all three panes including the foe preview canvases (same ctx-swap technique as `renderFoePreview`).

**Progression:** quota of bugs per level (`min(8 + level*4, 24 + level*2)` — matches the old linear curve through level 8, then softens so late levels are intense rather than long slogs; boss counts as 5, dragonfly 3). Each level has a fun rank name (`RANKS` array: HATCHLING → … → THE EXTERMINATOR, then ITSY BITSY LEGEND) shown in the HUD, at level start, and as "PROMOTED:" in the shop. Between levels an upgrade shop spends points (same number as score, tracked separately as `cash`): Swift Legs, Silk Range, Silk Glands (both meters), Strong Grip, Tough Silk, Venom Fangs, **Spread Silk** (v14: wider spread radius +28/tier and faster recharge −1.3s/tier, max 3), **Nest Instinct** (v15: tougher eggs +60hp/tier, faster hatch −2.5s/tier, bigger nest cap, max 3), Extra Heart. Once **all core upgrades are maxed**, an ADVANCED section appears (`SINKS`): Double Shot (3-way spread), Fortified Silk (stacking, +35% stuck time), Buy A Life (max 3, v25), Bug Bomb (B key — screen-wide shockwave: wraps every non-boss bug, bosses take exactly 1 hit). Hearts refill +1 per level clear. High score (`bughunt_hi`), lifetime "bugs debugged" (`bughunt_total`), the top-8 score board (`bughunt_scores`), the player's global-board name (`bughunt_name`), arena size (`bughunt_size`), world theme (`bughunt_theme`), music toggle (`bughunt_music`), and mute (`bughunt_mute`) all persist in localStorage under `bughunt_*` keys.

**Background (v22, themed v28):** `renderBg()` bakes (once per resize / level / theme change) a rich per-level sky. The deterministic RNG is seeded by `1337 + level * 7919` so every level has a unique layout. Layers, bottom to top: (1) a deep **radial gradient** whose core carries `bgTint()` (per-level hue via `bgHue()` → the active theme's `hue(lv)`); (2) **nebula clouds** — large soft radial-gradient blobs around the theme hue, very low alpha; (3) **three-tier starfield** — dim white motes, medium colored stars, and **bright stars** with glow halos (stored in `bgStars` for per-frame twinkling); (4) the orb-web motif (`bigWeb()`: 14 spokes + 9 wobbled rings) and two corner brand webs; (5) an **edge vignette**. On top, `draw()` adds per-frame: a **breathing core-glow**, **twinkling bright stars**, rare **shooting star** streaks, and **ambient dust** (hue-tinted motes follow the theme). `startLevel()` calls `renderBg()` so the sky shifts as you climb.

**World themes (v28, full environments v28.5):** a **WORLD picker row** on the menu (`th_*` buttons, same `.sizeBtn` styling as the arena row) selects one of five backdrops, persisted as `bughunt_theme` and applied instantly via `setTheme()` → `renderBg()`. Each theme is a complete scene painter (`BAKERS` table: `bakeCosmic/bakeNeon/bakeGarden/bakeEmber/bakeVoid`), signature `function(b, f, rnd, hue, lv)` — `b` is the baked background canvas, `f` an optional **foreground canvas** (`fgCvs`, drawn above the background but below entities; return `true` to enable it), and each painter registers animated particles into `themeAnim` for the per-frame living layer (`drawThemeAnim(t)` in `draw()`, budget ≤ ~40 primitives). Shared helpers: `skyRadial`, `starTiers(b,rnd,hue,density,yMax,cool)`, `nebulas(...,boost)`, `glowDot`. The scenes: **COSMIC** — ringed gas giant (banded, front/back ring passes), rebuilt spiral galaxy (bright core + defined arms + star grains), milky-way diagonal band with dust lane, shaded moons, asteroid belt arc, constellation lines, a derelict probe easter egg with a blinking beacon, near-field dark asteroid silhouettes in the FG corners; anims: star twinkle, shooting stars, beacon. **NEON** — synthwave: scanline sun (slats across the disc), two mountain ranges, city skyline with lit window panes + rooftop rim light, sun reflection column down the floor, converging grid verticals + faint concentric arcs (the floor is secretly a giant orb-web), FG palm silhouettes opposite the sun; anims: rolling grid horizontals, sun breathing, red antenna beacon. **GARDEN** — true crescent moon (sky-colored bite), far tree-line canopy, soft linear-gradient mist bands, two grass depths (far baked, near FG with dew glints), FG overhanging branches with leaf clusters + a dew-strung web fragment, glowing mushrooms, tiny flowers; anims: low-band drifting fireflies (dimmer than pickups, y ≥ 0.58H so they're never mistaken for the firefly bug), falling leaf, swaying seed stalks, mushroom glow pulse. **EMBER** — jagged seeded volcano with crater glow drawn behind the cone + face-clamped lava streams, a second distant volcano left, foothill silhouettes, smoke plume, basalt ground with glowing cracks (core alpha capped below gem pickups), coals, FG charred rocks + a scorched snag strung with a burnt web; anims: 12 rising sparks, crater pulse, breathing floor glow. **VOID (rebuilt v29.5: \u201cthe Great Web\u201d)** — you're hunting inside the torn web of something MUCH bigger: an enormous orb-web anchored beyond the top-left corner sweeps huge two-pass silk curves (sagging spokes + gap-torn rings whose ends curl; segments near the arena center are skipped via `safeR`), loose snapped threads, a pale watcher-light whose soft 3-pass beam lands in an elliptical floor pool, **two colossal rim-lit leg tips resting in a bottom corner** (the owner), swaying silk-wrapped **cocoons** hanging from the top edge (`cocoon` anim), low floor mist, heavy vignette; anims: cocoon sway, a ring pulse blooming from the beam pool, beam dust. The small `bigWeb` brand motif is skipped on void (the whole scene IS the web). **Easter eggs (v29):** every world hides something — VOID's watcher blinks **"BITZY" in morse code** (`watcher` anim builds an on/off timeline from `-... .. - --.. -.--`) and ~18% of levels **the owner opens a pale eye** in the corner shadow above its legs; COSMIC has the derelict probe with a blinking beacon (~60% of levels); NEON gets a rare **UFO flyby** (`ufo` anim, every 35-85s); GARDEN sometimes has a **pair of blinking eyes in the tall grass** (~35% of levels); EMBER spawns a **golden phoenix spark** that spirals skyward every 30-75s. Level-to-level hue rotation still happens *within* each theme's family (`THEMES[k].hue(lv)`/`sat` feed `bgHue()`/`bgTint()`). `?theme=x` forces a theme for testing. Dev tooling in the session scratchpad: `shoot-themes.js` (playwright screenshots of every theme via the local chromium at `/opt/pw-browsers/chromium-1194/chrome-linux/chrome`) and `perf-themes.js` (per-theme fps) — all themes hold 58-60fps at 1440×900.

**8-bit soundtrack (v29, expanded v33):** a zero-asset WebAudio **chiptune sequencer** (`SONGS`/`MUS`, ~90ms `setInterval` lookahead scheduler that stays sample-accurate on the AudioContext clock). Twelve tunes composed as step patterns (16th notes, MIDI numbers, `bars()`/`rep()` helpers; named chunk vars make phrases reusable): **"Silk Lullaby"** (menu — 16 bars/256 steps in A A B A' form: Am F C G verses, a Dm-Am-E-Am bridge, octave-shimmer reprise, 84bpm), **"Fugue in A Minor (for Synthesizers)"** (the hunt, ODD levels, after J.S. Bach — 16-bar/256-step form at 104bpm: exposition (subject → answer at the fifth over countersubject → subject in the triangle pedal) → middle entry in C major (subject +3) → sequential episode → stretto (subject in the lead, answer entering 8 steps later, subject fragment in the bass) → final V-i cadence; one quiet hat, no snare), **"Tarantella of the Black Widow"** (the hunt, EVEN levels — the spider dance itself: 6/8 spin at 132bpm, twelve 16th-steps per compound bar, 192 steps, Am with F-Dm-E turns, root/fifth leaping triangle bass, tambourine snare on beat two), **"The Molt King"** (boss levels — extended to 8 bars: the Dm Bb Gm A strain plus a B strain climbing to C#5; doubled 16th bass, dense hats). Plus three more hunt tunes (v29.9): **"March of the Thousand Hatchlings"** (`field3` — staccato Dm march at 112bpm, quarter-note walking triangle bass, clipped square motifs, rolling snare — tiny legs on parade), **"Waltz on a Single Thread"** (`field4` — Em waltz at 100bpm, 12-step 3/4 bars via `wBar`/`wPah` helpers: root on one, two chord-tone "pahs", long lilting lead, brushed hat on 2+3), **"Scuttle Boogie"** (`field5` — a real 12-bar blues in A at 124bpm: walking boogie triangle bass A-C#-E-F#-G, pentatonic blues licks with the b5, snare backbeat). **Five more classical-inspired tunes (v33), all public-domain compositions arranged for the chip voices:** **"In the Hall of the Spider Queen"** (`field6`, after Grieg — the stalking theme creeps in low over quarter-note root-fifth bass, climbs a fifth, sharpens into the raised-third variant, then breaks into an octave-up frenzy with rolled-chord stabs and a trill finale; percussion builds from lone quarter-hats to full kit across the 16 bars, 138bpm), **"Flight of the Bumblebee"** (`field7`, after Rimsky-Korsakov — chromatic 16th-note runs nearly every step at 152bpm over driving eighth-note bass; the swarm tune), **"Danse Macabre on Eight Legs"** (`field8`, after Saint-Saëns — 3/4 waltz at 126bpm, 12-step bars: the falling waltz theme in Gm, then the devil's tritone (A against Eb) sawing on beat pairs; waltz-pah arp on beats 2+3), **"Toccata of the Funnel-Web"** (`boss2`, after Bach's Toccata in D minor — the organ flourish falls through three octaves into the crypt, rolls the diminished chord, then churns like a pedal fugue over eighth-note D pedal bass; percussion enters at bar 5), **"Moonlight on the Web"** (`lull`, after Beethoven — the adagio's rolling triplets in C#m, one soft triangle note per step at 58bpm, whole-note bass, the dotted melody entering at bar 5; no percussion). **Rotation:** `HUNT_ROTATION` = all 8 field tunes, `musForState` picks `HUNT_ROTATION[(level-1) % 8]` per level; boss levels get the Molt King except every 10th level gets the Toccata (`level % 10 === 0 ? 'boss2' : 'boss'`); the tutorial now plays Moonlight (`lull`) instead of the fugue. `musForState()` (called from `setState` + once at boot) picks the tune: menu/gloss/controls/scores → menu, jukebox → leaves the player's pick spinning, play/shop/intro → per the rotation, over → stop. **Jukebox (v33):** a ♫ JUKEBOX ghost button on the menu opens `#ovJuke` (state `'jukebox'`), listing every tune from `TRACK_INFO` (key, display name, credit line like "after Edvard Grieg", MENU/HUNT/BOSS/TUTORIAL tag) as `.jukeRow` buttons rendered by `renderJuke()`; tapping a row spins it (`jukeSpin(k)` — auto-unmutes and force-enables music so the pick is audible, tapping the playing row stops it), the active row highlights red with a ▶. BACK returns to `jukeReturn` and `musForState` resumes the menu tune. Smoke checks: every SONGS key must appear in TRACK_INFO. Note: on your last heart the thin-mode strips the fugue to its pedal voice — bars 1-2 go silent before the bass enters, which reads as intentional dread. Voices: `musTone` (osc + exp-decay gain envelope) and `musNoise` (shared 0.25s noise buffer). **De-pop (v31.3):** every voice (music + `beep` SFX) now has a **linear attack ramp** (3-8ms) and a **linear tail to true zero** before its source stops — instant-gain starts were waveform discontinuities that popped like a maxed-out speaker (measured: the mix itself never clipped, peak 0.13). **De-pop II (v34):** a second, subtler pop survived the ramps — a GainNode defaults to **1.0**, and a source's first sample can render one sample BEFORE the `setValueAtTime(0)` event lands (source start times and AudioParam events round to the sample grid independently), leaking a single full-scale sample through unity gain. Offline renders showed isolated one-sample impulses of 0.67-0.83 amplitude (10x the music's real peak of ~0.08) at note starts; frequency of the race depends on tempo/step alignment, so the v33 tunes (152/126/138/58 bpm) hit it constantly. Fix: `g.gain.value = 0` at construction in all three voices (`musTone`/`musNoise`/`beep`) so the pre-automation intrinsic value is silence. Measured after: every song maxΔ ≤ 0.066 (normal band-limited waveform edges), peaks 0.08-0.11. Everything routes through a shared **master bus** (`outNode()`: DynamicsCompressor → 0.8 gain → destination) as insurance against summed peaks. **Audio controls:** a SOUND row on the menu — `snd_music` toggles `musicOn` (persisted `bughunt_music`; clicking it while hard-muted unmutes everything first) and `snd_sfx` toggles the master `muted` (same flag as the M key / pause button; `syncSound()` keeps the pills lit correctly). **Adaptive layer (v29.5):** `musMod()` (computed once per scheduler flush) re-voices the same patterns live from arena state — **rage** transposes everything +2 semitones and boosts the lead 1.35×; **your last heart** strips lead/arp/hats so only bass + snare thump like a heartbeat (not in the tutorial); an **enraged boss** drives percussion 1.35× and transposes +1; a **swarm** fills every hat step for 5s (`lastSwarmAt`). Tracks carry a `role` field (`bass/lead/arp/hat/snare`) so the modifiers know what to touch. Headless-testing note: the harness stubs `setInterval` (the scheduler would otherwise keep node's event loop alive forever) and the tutorial-bot must not re-wrap combo bugs every frame (each wrap extends the 60ms hit-stop freeze, so `update()`/`tutUpdate` would never run — a real hang, fixed in the test driver).

**Mobile layout (v29.6):** on touch devices `document.body` gets a `.touch` class and the action buttons split into **two thumb stacks**: right = WEB/RAGE/EGG (+DETACH while weaving, `#hudBR` as column-reverse so WEB sits at the thumb), left = AUTO/EAT/FIX (reparented into a JS-created `#hudBL`). Touch buttons grow to ≥44px targets (13px font, 96px min-width); the center HUD (`#hudB`: hearts + 4 meters + label) slims to `min(38vw, 150px)` so nothing overlaps the stacks on a 390px phone. Menu `.sizeRow`s flex-wrap (≤480px puts each row label on its own line) so all five WORLD buttons stay tappable. **Finger-occlusion fix:** in `setTarget`, touch pointers lift the follow-target 62px above the finger (easing to 0 near the bottom edge via `(H - y)/170` so the whole arena stays reachable) — aimed tap-shots still fire at the literal tap point. Mobile is screenshot-tested via `shoot-mobile.js` (390×844, `hasTouch: true`) in the session scratchpad.
...
**Versioning policy:** every shipped iteration bumps `GAME_VERSION` a whole number (v33 → v34 → …) — no point releases. Bump the label, the smoke-test assertion, and the docs together in the same commit.
...
**ADMIN mode (v27, instant playtesting):** `?dev=1` — or automatically in the dev-preview artifact, whose build script injects `window.__BUGHUNT_DEV__ = true` before the game script — turns on admin powers so new features can be played with immediately instead of earned. The version label gains `· ADMIN`. **Number-row hotkeys** (play state only): `1/2/3/4` equip SHOTGUN/LASER/GATLING/BAZOOKA (via `adminWeapon()` — the weapon **never expires**, HUD shows `∞`), `0` clears the weapon, `5` +1 bomb, `6` fills the rage meter, `7` egg ready, `8` full heal, `9` skips the level (`caught = quota`). A top-center **admin bar** of tiny pill buttons (`#adminBar`: SG/LZ/GT/BZ/✕/💣/RAGE/EGG/♥/SKIP) mirrors every hotkey so phone playtesting has the same powers — they're real `<button>`s, so `uiHit()` keeps clicks out of game input. `ADMIN` is off by default; production behavior is untouched without the flag.

**Dev preview (playtest before merging):** `game.html` is fully self-contained, so the work-in-progress build is published as a Claude Artifact the user can play immediately on any device — no merge to `main` needed. The flow: a build script in the session scratchpad strips game.html's outer `<!DOCTYPE>/<html>/<head>/<body>` wrapper (the artifact supplies its own), retitles it "Bug Hunt (dev build)", and the Artifact tool publishes it. **Redeploy to the same URL after every change** by passing `url: 'https://claude.ai/code/artifact/466c6428-04be-465a-a44f-4f2d04d952f0'` to the Artifact tool (a fresh session mints a new URL without it; if the artifact was deleted, publish under a NEW file basename to mint a fresh one — the old path stays bound to the dead artifact). Keep the 🕷️ favicon stable. Caveats in the sandboxed preview: the Supabase global-board fetch is blocked by CSP (fails gracefully to the local board) and localStorage may not persist — both fine for playtesting; everything else (canvas, input, audio) works.
...
- State machine: `menu | play | shop | pause | over | intro | gloss | controls | scores | jukebox` — DOM overlays per state, canvas renders always

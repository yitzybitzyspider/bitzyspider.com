...
**Nest — eggs & spiderlings (v15):** press **N (or the touch EGG button)** to lay an **egg sac** at your position (costs `EGG_COST` 60 webSilk). The sac is vulnerable on the ground — any non-webbed, non-boss bug that reaches it bites it (`EGG_BITE` 34 hp/sec, drains its health ring) and **bugs actively drift toward the nearest egg** to raid it (a mild homing pull layered on their own AI in `updateBugs`, so flies/beetles/wasps come for the nest while moths/fireflies still prefer your web). **Protect the nest with your web:** a strand-hub (`HUB_R`) over the egg cuts bite damage to 32% (a *fortified nest*), and strands around it stop raiders before they reach it — so the loop is weave a web, lay eggs inside it, defend until they hatch. After `eggHatchMs()` (~16s, faster with NEST) a surviving egg **hatches into a spiderling** (`spawnSpiderling`) — a tiny autonomous hero-spider (`updateSpiderlings`/`drawSpiderling`, scale 0.34, `SPIDERLING_SP` 138) that chases the nearest catchable bug and grabs it at its mouth (counts toward quota + feeds webSilk like your own catches; no wrap-freeze). The nest holds `maxBrood()` = 2 + brood eggs+spiderlings combined (laying is gated on this). Eggs/spiderlings **persist across levels** within a run and reset on `startGame`/`toMenu`/tutorial. The **NEST INSTINCT** shop upgrade (`brood`, max 3) toughens eggs (+60 hp/tier), speeds the hatch (−2.5s/tier), and raises the nest cap. HUD shows `· EGGS n · BROOD n`.

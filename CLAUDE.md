...
**Auto-weaver (Q / AUTO button):** the spider *walks* an outward spiral around the point where it was toggled (the weaver drives `target`; `lastInputAt` is refreshed so idle-wander stays off), pinning a strand segment behind it every ~46px exactly like manual weaving — each segment costs webSilk. Each time the walk crosses one of 6 fixed spoke angles (`AUTO_SPOKES`) it ties a radial connector inward — to the shared center hub on the first row (which becomes a true hub with rings + slow field), to the previous row after that. The angles repeat every row, so the spokes align into continuous radials and the web visibly grows out from its center. Stops at ~3.2 rows ("web complete!"), on silk-out, on the strand cap, or **when the player moves the mouse / drags** (a 600ms grace after toggling ignores jitter). A faint dashed guide shows the upcoming spiral path.

**Menu/pause UI:** the start menu and pause overlay share one icon-based controls grid (`CTRL_ROWS` → `renderControls()` into `#menuControls` and `#pauseControls`), with `.secHead` section labels (HOW TO PLAY / KNOW YOUR BUGS) and a compact `.tipLine` for the meter/structure rules. Touch and desktop get different rows.

**Field Guide (v16):** a comprehensive glossary overlay (`#ovGloss`, state `'gloss'`) accessible from both the menu and pause screen via FIELD GUIDE ghost buttons. Three tabbed panes (`.glossTab` buttons switching `.glossPane` visibility): **BESTIARY** — every bug type with a live `drawBug` canvas preview (52×52), name (colored), threat tags (RAIDER/STINGER/BURNER/WEB-EATER/BOSS), the `FOE_INFO` description, and stat line (unlock level, escape time, weight class, special notes including boss drops). **UPGRADES** — all 9 core upgrades with icons (`UPG_ICONS`), pip count, and plain descriptions (`UPG_DESC`), plus the 4 advanced sinks. **MECHANICS** — 8 reference cards covering silk meters, strand support rules, heavy bugs, hubs, combos, brood/nest, difficulty scaling, and boss drops. The `glossReturn` variable remembers whether to return to `'menu'` or `'pause'` on close. Rendering happens in `renderGlossary()` which populates all three panes including the foe preview canvases (same ctx-swap technique as `renderFoePreview`).
...
**Background (v14):** the arena is no longer flat black. `renderBg()` bakes (once per resize / level) a deep radial gradient whose core carries a faint **per-level color tint** (`bgTint()` rotates the hue ~33° each level at 38% sat / 7% light), a deterministic **starfield** (~`W*H/9000` faint motes), a large very-low-alpha **orb-web motif** (`bigWeb()`: 14 spokes + 9 wobbled capture rings from the upper-center), and the two corner brand webs. On top, `draw()` adds a slow **breathing core-glow** (a per-frame radial gradient pulsing on `sin(t*0.0006)`) so the dark arena reads as alive without re-rendering the bake. `startLevel()` calls `renderBg()` so the tint shifts as you climb.
...
- State machine: `menu | play | shop | pause | over | intro | gloss` — DOM overlays per state, canvas renders always
